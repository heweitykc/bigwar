...
public class EchoAsteroidBelt : EchoGameObject
{
	...
	public void MakeAsteroidBelt()
	{
		...
		asteroid_parent.GetComponent<Renderer>().sharedMaterial = asteroid_prefab1.GetComponent<Renderer>().sharedMaterial;
		...
		for ( loop = 0; loop < asteroid_count; loop++ )
		{
			...
			if ( Random.Range ( 0, 100 ) < 70 || asteroid_prefab2 == null )
			{
				...
				aster.GetComponent<Renderer>().sharedMaterial = asteroid_prefab1.GetComponent<Renderer>().sharedMaterial;
			}
			else
			{
				aster = (GameObject)Instantiate(asteroid_prefab2 );
				aster.GetComponent<Renderer>().sharedMaterial = asteroid_prefab2.GetComponent<Renderer>().sharedMaterial;
			}
