...
public class EchoGameObject : MonoBehaviour
{
	...
	public Material                         alternateMaterial           = null;
...
//==========================================================================
	public bool Init ( GameObject igo = null, bool ifixScale = false, bool iaddchildren = false, bool iactiveflag = true, bool irendererenabled = true )
	{
		...
		{
			echo_Renderer = GetComponent<Renderer>();
		}
		else
		{
			echo_Renderer = echo_skinnedMeshRenderer.GetComponent<Renderer>();
		}
		...
		if ( echo_Renderer )
		{
			mainMaterial                    = echo_Renderer.sharedMaterial;

			_shaderProperties = new List<EchoShaderProperty>(32);

			InitShaderProperties( mainMaterial );
			InitShaderProperties( alternateMaterial );
...
//------------------------------------------------------------------------------
	public void MakeFullOutlineMesh()
	{
		...
		go.active = false;
#endif
		go.GetComponent<Renderer>().materials           = echo_Mat1;
...
//===========================================================================
	private void EchoBurnMeshScale()
	{
		...
		if ( GetComponent<Collider>() != null )
		{
			...
			{
				BoxCollider c = GetComponent<Collider>() as BoxCollider;
			...
			{
				CapsuleCollider c = GetComponent<Collider>() as CapsuleCollider;
		...
		if ( GetComponent<Collider>() != null && GetComponent<Collider>().GetType() == typeof(MeshCollider) )
		{
			MeshCollider c		= GetComponent<Collider>() as MeshCollider;
			c.sharedMesh		= mesh;
		}
	}
...
//&===========================================================================
	...
	{
	  	GameObject newgo = UnityEngine.Object.Instantiate (this.gameObject) as GameObject;
		
		newgo.layer						= gameObject.layer;
		newgo.GetComponent<Renderer>().sharedMaterial 	= GetComponent<Renderer>().sharedMaterial;
	...
	public void Outline ( bool flag )
	{
		...
		if ( echoOutlineMesh != null )
		{
			if ( flag )
			{
				...
				echo_OutlineMeshObj.GetComponent<Renderer>().materials           = echo_Mat1;
				SetMaterialsRenderQueueOutline ( echo_OutlineMeshObj.GetComponent<Renderer>().materials, 2449 );
...
//&===========================================================================
	public void SwapMaterial()
	{
		...
		{
			materialId = 1;
			echo_Renderer.sharedMaterial = alternateMaterial;
		}
		else
		{
			materialId = 0;
			echo_Renderer.sharedMaterial = mainMaterial;
		}
	}
...
//&===========================================================================
	public EchoGameObject AddShield ( Material imat = null, float iscale = 1.2f )
	{
		...
		shield.GetComponent<Renderer>().sharedMaterial	= imat;
...
//===========================================================================
	private EchoShaderProperty AddShaderProperty ( Material isharedMat, string sname )
	{
		...
		{
			foreach ( EchoShaderProperty espl in _shaderProperties )
	  		{
				if ( espl.name == sname )
				{
					esp = espl;
...
//===========================================================================
	public void InitShaderProperties ( Material imat )
	{
		if ( imat == null )
			return;

		//-------
		_MainTex_ST  		= AddShaderPropertyVector4 ( imat, "_MainTex_ST" );
		_Color  			= AddShaderPropertyVector4 ( imat, "_Color" );
		_echoRGBA			= AddShaderPropertyVector4 ( imat, "_echoRGBA" );
		_echoAlpha			= AddShaderPropertyFloat ( imat, "_echoAlpha" );
  		_echoMix			= AddShaderPropertyFloat ( imat, "_echoMix" );
		_echoUV				= AddShaderPropertyVector4 ( imat, "_echoUV" );
		_echoScale			= AddShaderPropertyVector4 ( imat, "_echoScale" );
		_echoHitMix0		= AddShaderPropertyFloat ( imat, "_echoHitMix0" );
		_echoHitMix1		= AddShaderPropertyFloat ( imat, "_echoHitMix1" );
		_echoHitMix2		= AddShaderPropertyFloat ( imat, "_echoHitMix2" );
		_echoHitMix3		= AddShaderPropertyFloat ( imat, "_echoHitMix3" );
		_echoHitVector0 	= AddShaderPropertyVector4 ( imat, "_echoHitVector0" );
		_echoHitVector1 	= AddShaderPropertyVector4 ( imat, "_echoHitVector1" );
		_echoHitVector2 	= AddShaderPropertyVector4 ( imat, "_echoHitVector2" );
		_echoHitVector3 	= AddShaderPropertyVector4 ( imat, "_echoHitVector3" );
	}
...
//&-----------------------------------------------------------------------------
	public void ShaderSet_echoHitVectorOn ( Vector3 ivec, int ihitnum )
	{
		...
		ivec = GetComponent<Renderer>().worldToLocalMatrix * ivec;
