...
public class BattleManager : MonoBehaviour {
	...
	void onItemChoosed(object data){
		...
		if(_selectedItem == null){
            ...
            Debug.Log("选中：" + _selectedItem.data.pname);
		} else {
			if(AppGlobal.itemMgr.selectedItem._soldierId >= 0){
                ...
                AppGlobal.itemMgr.selectedItem.setSoldier(
                    _selectedItem._soldierId, 
                    _selectedItem.data
                );                
                _selectedItem.clear();
				_selectedItem = null;
    ...
    {        
        ...
        BattleItem[] items = (i >= 0 ? team0 : team1);
        i = Array.IndexOf(items, item);
        Debug.Log("i=" + i + "," + item.data.pname);
        i++;
        int[] ss = null;
        if (i == 1 || i == 4 || i == 7)
        {
        ...
        var enemies = items == team0 ? team1 : team0;
        foreach (int s in ss)
        {
            BattleItem itm = enemies[s - 1];
            if (itm._soldierId >= 0)
            {
                return itm;
            }
